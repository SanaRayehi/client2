...
public class ChatRoom extends Stage {
    ...
    public ChatRoom(Chat chat, String senderUsername) {
        ...
        Label label;
        if(ClientSide.checkUserActivity(chat.getUsername())) {
            label = new Label(chat.getUsername()+" (online)");
        }else {
            label = new Label(chat.getUsername()+" (offline)");
        }
        label.setTextAlignment(TextAlignment.CENTER);
        label.setStyle("-fx-background-color: aqua");
        pane.setAlignment(Pos.CENTER);
//        SenderMessageTemplate senderMessageTemplate = new SenderMessageTemplate();
//        senderMessageTemplate.prefWidthProperty().bind(scrollPane.widthProperty().subtract(20));
//        pane.getChildren().addAll(senderMessageTemplate, new ReceiverMessageTemplate());
        ...
        for(Message message: messageList) {
            if(message.getUsername().equals(senderUsername)) {
                ...
                if(message.getMessage()!=null) {
                    ...
                    senderFileTemplate.progressBar.setProgress(0);
    ...
    private void startConnection(String username, int chatid) {
        ...
        try {
            ...
            ListenerService listenerService = new ListenerService(socket.getInputStream(), pane, scrollPane, username, chatid);
