    ...
    public App(UserBaseInfoDto userBaseInfoDto) {
        ...
        settingMenuItem.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                Setting setting = new Setting(user, App.this);
                setting.show();
            }
        });
            ...
            public void handle(KeyEvent event) {
                if(event.getCode() == KeyCode.ENTER) {
                    try {
                        ...
                        boolean found = false;
                        for(Chat chats: userBaseInfoDto.getChats()) {
                            if(chats.getUsername().equals(chat.getUsername())){
                                found = true;
                                ChatRoom chatRoom = new ChatRoom(chat, user.getUsername());
                                chatRoom.show();
                                return;
                        ...
                        if(found == false) {
                            userBaseInfoDto.getChats().add(chat);
                            UserEntry entry = new UserEntry(chat);
                            pane.getChildren().add(entry);
